                ...
                if response.status == 200:
                    ...
                    for item in arr:
                        try:
                            ...
                            events.append(normalize_event(name, "Devpost", link, start, end, tags, loc, prize, "Free", "Students"))
                        except Exception: continue
        except Exception: pass
    return events
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        # Launch all infinite un-capped runners concurrently
        tasks = [
